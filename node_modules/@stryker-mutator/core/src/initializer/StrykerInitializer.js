...
class StrykerInitializer {
    constructor(log, out, client, strykerPresets, configWriter, inquirer) {
        this.log = log;
        this.out = out;
        this.client = client;
    ...
    async initiateCustom(configWriter) {
        ...
        const selectedTestFramework = selectedTestRunner && !CommandTestRunner_1.default.is(selectedTestRunner.name)
        ...
        const selectedPackageManager = await this.selectPackageManager();
        const npmDependencies = this.getSelectedNpmDependencies([selectedTestRunner, selectedTestFramework, selectedMutator]
    ...
    installNpmDependencies(dependencies, selectedOption) {
        ...
        const cmd = selectedOption.name === "npm" /* Npm */
    ...
    async fetchAdditionalConfig(dependencies) {
        return objectUtils_1.filterEmpty(await Promise.all(dependencies
