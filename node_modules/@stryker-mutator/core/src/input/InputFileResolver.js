...
const util_3 = require("@stryker-mutator/util");
const util_4 = require("@stryker-mutator/util");
...
class InputFileResolver {
    constructor(log, { mutate, files }, reporter) {
        this.log = log;
    ...
    async resolveFilesUsingGit() {
        try {
            const { stdout } = await util_1.childProcessAsPromised.exec('git ls-files --others --exclude-standard --cached --exclude .stryker-tmp', { maxBuffer: 10 * 1000 * 1024 });
            ...
            throw new util_2.StrykerError(util_4.normalizeWhitespaces(`Cannot determine input files. Either specify a \`files\`
    ...
    readFile(fileName) {
        return util_3.fsAsPromised
            ...
            .catch(error => {
            if ((util_3.isErrnoException(error) && error.code === 'ENOENT') ||
                error.code === 'EISDIR') {
